...
BEGIN_MESSAGE_MAP(CRouterDlg, CDialogEx)
	...
	ON_WM_DESTROY()
	ON_LBN_SELCHANGE(IDC_LIST1, &CRouterDlg::OnLbnSelchangeList1)
END_MESSAGE_MAP()
...
BOOL CRouterDlg::OnInitDialog()
{
	...
	*/
	...
	m_Log.AddString(CString(m_selectdevs->name));			//显示该网络接口设备的名字
	m_Log.AddString(CString(m_selectdevs->description));	//显示该网络接口设备的描述信息
	...
	for(a = m_selectdevs->addresses; a; a = a->next)	
	{
		if (a->addr->sa_family == AF_INET)
		{
			...
			ipaddr.IPMask = (((struct sockaddr_in *)a->netmask)->sin_addr.s_addr);
			IfInfo[0].ip.Add(ipaddr);
			m_Log.AddString(L"IP地址：" + IPntoa(ipaddr.IPAddr));
			j++;	
		}
	}
	...
	if ( (IfInfo[0].adhandle = pcap_open(IfInfo[0].DeviceName, // 设备名
			                       65536,                          // 最大包长度
					               PCAP_OPENFLAG_PROMISCUOUS,      // 混杂模式
						           1000,                           // 超时时间
							       NULL,                           // 远程认证
		...
		{
			// 错误，显示错误信息
			MessageBox(L"接口打开失败："+CString(errbuf),MB_OK);
		}

	// 开启数据包捕获线程，获取本地接口的MAC地址，线程数目为网卡个数
	CWinThread* pthread;
	pthread = AfxBeginThread(CaptureLocalARP, &IfInfo[0], THREAD_PRIORITY_NORMAL);
	if(!pthread)
	{
		MessageBox(L"创建数据包捕获线程失败！");
		PostMessage(WM_QUIT, 0, 0);
	...
	setMAC(IfInfo[0].MACAddr, 0);

	// 为得到真实网卡地址，使用虚假的MAC地址和IP地址向本机发送ARP请求
	setMAC(srcMAC, 66);							// 设置虚假的MAC地址
	srcIP = inet_addr("112.112.112.112");		// 设置虚假的IP地址
	ARPRequest(IfInfo[0].adhandle, srcMAC, srcIP, IfInfo[0].ip[0].IPAddr);

	Sleep(3000);	

	m_Log.AddString(L"MAC地址：" + MACntoa(IfInfo[0].MACAddr)); 

	// 初始化路由表并显示
	RouteTable_t rt;
	i = 0;
	for (j = 0; j < IfInfo[0].ip.GetSize(); j++)
	{
		rt.IfNo = i;
		rt.DstIP = IfInfo[0].ip[j].IPAddr  & IfInfo[0].ip[j].IPMask;
		rt.Mask  = IfInfo[0].ip[j].IPMask;
		rt.NextHop = 0;	// 直接投递
		RouteTable.AddTail(rt);
		m_RouteTable.InsertString(-1, IPntoa(rt.Mask) + " -- " + IPntoa(rt.DstIP) + " -- " + IPntoa(rt.NextHop) + "  (直接投递)");
	}
...
void IPPacketProc(IfInfo_t *pIfInfo, struct pcap_pkthdr *header, const u_char *pkt_data)
{
	...
	else
	{
		...
		if (IPLookup(sPacket.TargetIP, IPf->FrameHeader.DesMAC)) 
		{
			...
			//发送ICMP应答
			//ICMPPacketProc(pIfInfo, 8, 0, pkt_data);
...
void CRouterDlg::OnLbnSelchangeList1()
{
	// TODO: 在此添加控件通知处理程序代码
}
